...
var wss = "wss://your-fastlynode-endpoint";
const secretKey = "Your secret key, please make sure to keep it safe"
    ...
    if(value>30) {
      ...
      if(result.length>0){
        ...
        if(result[1].length>0){
          ...
          const buyGasPrice = calculate_gas_price("buy",gasPrice)
          const sellGasPrice = calculate_gas_price("sell",gasPrice)
          // after calculating the gas price we buy the token
          console.log("going to buy");
          await buyToken(account,tokenAddress,gasLimit,buyGasPrice)
          // after buying the token we sell it 
          console.log("going to sell the token");
          await sellToken(account,tokenAddress,gasLimit,sellGasPrice)
        }
      }
    }
    }
